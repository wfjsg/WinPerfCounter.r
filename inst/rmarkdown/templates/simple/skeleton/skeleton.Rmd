...
decorateVerticalLines <- function(plot) {
  if( length(eventVerticalLines) == 0 ){
    return(plot);
  }
...
cpu_chart <- decorateVerticalLines(cpu_chart)
...
memory_chart <- decorateVerticalLines(memory_chart) 
...
iops_chart <- decorateVerticalLines(iops_chart)
...
iobytes_chart <- decorateVerticalLines(iobytes_chart)
```
