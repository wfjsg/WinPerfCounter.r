...
data <- WinPerfCounter.readcsv("../../../../../tests/testthat/test2.csv")
...
```{r}
WinPerfCounter.formattable(WinPerfCounter.summary(cpu))
```

## Summary(Memory)[GB]
```{r}
WinPerfCounter.formattable(WinPerfCounter.summaryInGB(memory))
```
...
```{r}
statics <- WinPerfCounter.summary(cpu)
WinPerfCounter.formattable(statics)
```
