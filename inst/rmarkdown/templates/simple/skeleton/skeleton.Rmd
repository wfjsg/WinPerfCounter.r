...
# 間引き（サンプリング周波数を落とす）: 沢山プロットすると時間がかかるので。
if( viewPlotLimit > 0 ){
  ...
  if( rate > 1.0 ){
    ...
    time.lap <- c(time.lap, list(Resampling = proc.time()))
    data <- data[pickNum,]
  }
}
```
...
p.iops <- WinPerfCounter.Process.Metric.IOPS(data, processName)
iops_chart <- WinPerfCounter.Process.PlotIOPS(p.iops, processes, sysenv.memory, processName)
p.iobytes <- WinPerfCounter.Process.Metric.IOBytes(data, processName)
