...
# チャートの設定を変える。CPUは1コアあたりの使用率(4コアだと400%になる)。メモリの単位は[GB]
processes <- list(
  explorer = list( 
...
eventVerticalLines <- as.POSIXct(c(
  "2018/01/07 21:07:00",
  "2018/01/07 21:10:20"
), format="%Y/%m/%d %H:%M:%OS")

eventRange <- as.POSIXct(c(
  "2018/01/07 21:07:05", "2018/01/07 21:09:00",
  "2018/01/07 21:12:00", "2018/01/07 21:12:30"
), format="%Y/%m/%d %H:%M:%OS")

eventRangeStart <- eventRange[c(TRUE, FALSE)]
eventRangeEnd <- eventRange[c(FALSE, TRUE)]

decorate <- function(plot) {
  if( length(eventVerticalLines) > 1 ){
    plot <- plot + geom_vline(xintercept = eventVerticalLines, alpha = 0.5, color = "coral2")
  }
  print("annotate1")
...
```{r tidyTotal}
# cpu <- WinPerfCounter.SampleAtMinute( WinPerfCounter.Metric.CPU(data) )
cpu <- WinPerfCounter.Metric.CPU(data, resampleInterval)
cpu_chart <- ggplot(cpu, aes(x=Timestamp, y=value)) + ggtitle("System CPU")
cpu_chart <- cpu_chart + geom_point(aes(colour = metric), alpha = 0.7) + ylim(0.0, 100.0)
cpu_chart <- cpu_chart + theme_gray()
cpu_chart <- decorate(cpu_chart)

cpu_chart <- cpu_chart + annotate("rect", xmin = eventRangeStart, xmax = eventRangeEnd, ymin = 0, ymax =65000, 
                                  fill = "yello")
    

# memory <- WinPerfCounter.SampleAtMinuteGB( WinPerfCounter.Metric.Memory(data) )
...
memory_chart <- decorate(memory_chart) 
...
memory_chart <- memory_chart + geom_point(aes(colour = metric), alpha = 0.7) + ylim(0, sysenv.memory.chartlimit)
...
iops_chart <- WinPerfCounter.Process.PlotIOPS(p.iops, processes, sysenv.memory, processName)
iops_chart <- decorate(iops_chart)

p.iobytes <- WinPerfCounter.Process.Metric.IOBytes(data, processName, resampleInterval)
iobytes_chart <- WinPerfCounter.Process.PlotIOBytes(p.iobytes, processes, sysenv.memory, processName)
iobytes_chart <- decorate(iobytes_chart)
```
