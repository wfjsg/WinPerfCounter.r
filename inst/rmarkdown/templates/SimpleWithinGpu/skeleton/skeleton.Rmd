...
```{r todyGPU}
gpu <- list()
gpu[[1]] <- WinPerfCounter.Metric.GPU_Memory(data, resampleInterval, gpu.list[1])
gpu[[2]] <- WinPerfCounter.Metric.GPU_Memory(data, resampleInterval, gpu.list[2])
```

## GPU1 Memory[MB]

```{r showGpu1Memory}
statistics <- WinPerfCounter.summaryInMB(gpu[[1]])
WinPerfCounter.formattable(statistics)
```
...
```{r showGpu2Memory}
statistics <- WinPerfCounter.summaryInMB(gpu[[2]])
WinPerfCounter.formattable(statistics)
```
...
gpu_chart1 <- WinPerfCounter.System.DrawGpuMemory(gpu[[1]], plotConfig, gpu.list[1])
gpu_chart2 <- WinPerfCounter.System.DrawGpuMemory(gpu[[2]], plotConfig, gpu.list[2])
plot_grid(gpu_chart1, gpu_chart2, ncol=1, align="v")
```
